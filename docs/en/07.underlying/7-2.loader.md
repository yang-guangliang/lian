...
SymbolGraphLoader：
- 处理混合节点图：节点既包括语句 ID，也包括 `SymbolDefNode`
- 根据边的方向区分"定义"边和"使用"边
...
- 存储调用点的参数映射关系
- `ParameterMapping` 包含参数索引、状态 ID、访问路径等
...
问题：分析结果是基于GIR的（语句 ID），但用户需要看源代码。
...
2. 按文件路径过滤规则（避免无效匹配）
3. 遍历每个单元的 GIR，对每条语句：
   - 转换为可读的 GIR 字符串（`readable_gir.get_gir_str()`）
   - 使用正则表达式匹配符号名（`\b` 单词边界避免误匹配）
   - 如果规则指定了行号，额外检查行号是否匹配
4. 返回匹配的语句 ID 集合

### 6-3 调用关系统一查询

问题：不同阶段的调用关系存储方式不同（P2 用图，P3 用路径），查询接口也不同。
...
- `match_by`：`"name"`（返回方法名集合）/ `"id"`（返回方法 ID 集合）
- `entry_point_id`：指定入口点（仅对 P3 有效）
...
- 支持按导入名称、边类型（内部/外部）、语句 ID 等属性查询
- 返回边和节点的组合`(edge, node)`

## 7 总结：

语义存储和检索框架实现了高效、灵活、可维护的数据管理，在可控的内存和 I/O 开销下，为高精度全局分析提供数据管理基础。