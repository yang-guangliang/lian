...
- 如果是name()格式，而name是一个函数名，那么可以直接在CG添加一条边，即（当前函数，name函数）
- 如果是receiver.name()格式，常见的方法是引用类层级关系或者通过类型，定位receiver.name的函数原型，那么也可以在CG中添加一条边，即（当前函数，receiver.name函数原型）。特别的是，super.name()、this.name()、self.name()等，需要应用类层级来定位。
- 此外，还有些隐私的函数调用，例如初始化函数、运算符加载函数等，需要特殊处理。

当然，现实情况会更加复杂，尤其是在回调、高阶函数、lamda算子、闭包函数存在的情况下，那么就往往需要应用更加高级的分析方法来构建更加完整的CG。
