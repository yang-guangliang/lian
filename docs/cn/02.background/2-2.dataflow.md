...
- **∅** 表示空集,即该指令不修改或使用任何变量
- 对于**赋值指令**(assign),左值被修改,右值被使用
- 对于**变量声明**，不使用或修改任何值；也有的分析框架会把被声明的变量当作被修改以便于分析，因此这主要取决于分析策略
- 对于**数组操作**:
  - array_write: 数组本身被修改,同时需要使用数组引用、索引和源值
  - array_read: 目标变量被修改,需要使用数组引用和索引
- 对于**对象字段操作**:
  - field_write: 对象被修改,需要使用对象引用和源值
  - field_read: 目标变量被修改,需要使用对象引用和字段
  - 字段名(field)可作为常量,不计入use集合
- 对于**函数调用**:
  - 有返回值时,目标变量被修改
  - 所有参数和函数引用都被使用
  - 函数调用可能有副作用,影响全局变量或堆对象,需在过程间分析中特别处理
- 对于**控制流指令**:
  - return: 不修改变量,使用返回值(如果有)
  - if/goto: 不修改变量,会使用返回值


其中，对于复杂对象的def/use建模，需要根据分析的精度决定建模方式:
...
在具体分析中，若无法精确区分array[index]或receiver.field，则可退化为弱更新，以保证分析的**保守性soundness**，即不会遗漏任何可能的数据流，这个时候的分析结果可称之为**安全safe**。
...
数据流分析还有用来做非常多的任务。一个典型的数据流分析任务通常包含多种要求，通过以下多种分析策略选择，达成最终结果。

| 分析任务| 任务解释 | 分析方向 | 分析类型 | 状态含义 | 状态元素 | Meet 运算 | 初始值 |
| ----|----|----|----|-----|-----|-----|---|
| **定义可达性**(Reaching Definitions) | 对于stmt还有哪些变量def可用  | 前向   | May  | 某程序点可达得def| 变量def(stmt, var) | ∪（并集）   | ∅   |
| **活跃变量**(Live Variables)   | 对于stmt有哪些变量在未来还会被使用use | 后向   | May  | 指令之后use变量      | 变量    | ∪（并集）   | ∅   |
| **到达使用**(Reaching Uses)    | use被哪些def到达  | 后向   | May  | 对于每个use点,分析哪些def可能提供值       | 使用点             | ∪（并集）   | ∅   |
| **可用表达式**(Available Expressions) | 对于stmt有哪些表达式可用  | 前向   | Must | 已经计算过且未失效的表达式 | 表达式             | ∩（交集）   | 全集  |
| **必忙表达式**(Very Busy / Anticipated) | 对于stmt有哪些表达式在未来还会被使用use | 后向   | Must | 指令之后一定会被使用的表达式    | 表达式             | ∩（交集）   | 全集  |


#### 分析方向（Forward / Backward）
...
ICFG概念可以参照[2-1.程序分析基础](../2-1.basics.md)。